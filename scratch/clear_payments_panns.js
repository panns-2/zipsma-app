const admin = require('firebase-admin');

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert({
            projectId: 'zip-sma',
            clientEmail: 'firebase-adminsdk-fbsvc@zip-sma.iam.gserviceaccount.com',
            privateKey: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'.replace(/\\n/g, '\n')
        })
    });
}

const db = admin.firestore();

async function runOption1CleanUp() {
    const schoolId = 'PANNS290';
    console.log(`Starting Option 1 clear payments script for school: ${schoolId}...`);
    
    // Get all students for the school
    const snapshot = await db.collection('students')
        .where('schoolId', '==', schoolId)
        .get();
        
    console.log(`Found ${snapshot.docs.length} students total.`);
    
    let totalPaymentsRemoved = 0;
    let studentsUpdatedCount = 0;
    
    const batch = db.batch();
    
    for (const doc of snapshot.docs) {
        const data = doc.data();
        const ledger = data.ledger || [];
        
        // Filter out payments (where credit is greater than 0)
        const updatedLedger = ledger.filter(transaction => {
            const isPayment = Number(transaction.credit) > 0 || 
                              (transaction.category && transaction.category.toLowerCase().includes('payment')) ||
                              (transaction.categoryId && transaction.categoryId.toLowerCase().includes('payment'));
            if (isPayment) {
                totalPaymentsRemoved++;
                return false; // Remove this transaction
            }
            return true; // Keep this transaction (it's a charge/debit or adjustment)
        });
        
        const removedInThisStudent = ledger.length - updatedLedger.length;
        if (removedInThisStudent > 0) {
            console.log(`Student ${data.name || doc.id}: removing ${removedInThisStudent} payments. Ledger size goes from ${ledger.length} to ${updatedLedger.length}`);
            batch.update(doc.ref, { ledger: updatedLedger });
            studentsUpdatedCount++;
        }
    }
    
    if (studentsUpdatedCount > 0) {
        console.log(`Committing updates for ${studentsUpdatedCount} students...`);
        await batch.commit();
        console.log(`Successfully removed ${totalPaymentsRemoved} payments across ${studentsUpdatedCount} students.`);
    } else {
        console.log("No payment transactions found to remove.");
    }
}

runOption1CleanUp().catch(console.error);
