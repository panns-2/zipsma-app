
const admin = require('firebase-admin');

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert({
            projectId: 'zip-sma',
            clientEmail: 'firebase-adminsdk-fbsvc@zip-sma.iam.gserviceaccount.com',
            privateKey: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'.replace(/\\n/g, '\n')
        })
    });
}

const db = admin.firestore();

async function checkSchoolCredentials() {
    console.log('Checking credentials for PANNS290...');
    const doc = await db.collection('schools').doc('PANNS290').get();
    if (!doc.exists) {
        console.log('School PANNS290 not found.');
        return;
    }
    const data = doc.data();
    console.log(`Hubtel SMS Client ID: ${data.hubtelSmsClientId ? 'Present' : 'MISSING'}`);
    console.log(`Hubtel SMS Client Secret: ${data.hubtelSmsClientSecret ? 'Present' : 'MISSING'}`);
    console.log(`Hubtel Sender ID: ${data.hubtelSenderId || 'Not Set'}`);
}

checkSchoolCredentials().catch(console.error);
